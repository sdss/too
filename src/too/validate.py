...
def calculate_offsets(
    ...
    magnitudes = targets[
        ["g_mag", "r_mag", "i_mag", "z_mag", "gaia_bp_mag", "gaia_g_mag", "gaia_rp_mag"]
    ].to_numpy()
...
def bn_validation(
    targets: polars.DataFrame, design_mode: str, observatory: str = "APO"
) -> np.ndarray:
    """
    ...
    bn_maps_boss_file = "{BN_HEALPIX}/{design_mode}_{instrument}_bn_healpix.pkl".format(
        BN_HEALPIX=BN_HEALPIX, instrument="boss", design_mode=design_mode
    )
    ...
    bn_maps_apogee_file = (
        "{BN_HEALPIX}/{design_mode}_{instrument}_bn_healpix.pkl".format(
            BN_HEALPIX=BN_HEALPIX, instrument="apogee", design_mode=design_mode
        )
    )
    ...
    delta_ra, delta_dec, offset_flag = calculate_offsets(
        targets, design_mode, observatory=observatory
    )
